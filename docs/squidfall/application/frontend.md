...
**Step 11.** Create a `api` and `copilotkit` directory under in the `app` directory.  
```bash
mkdir -p app/api/copilotkit
```

**Step 12.** Add a file called `route.txt` in under the `app/api/copilot/` directory path you just created. and add the content below to it.   
```ts
import {
  CopilotRuntime,
...
**Step 13.** Open the file called `layout.tsx` under the `app` directory and replace its content with the code below.
```ts
...
**Step 14.** Open the file called `page.tsx` under the `app` directory and replace its content with the code below.
```ts
...
**Step 15.** Open the file called `next.config.ts` under the `squidfall` directory and replace its contents with the code below.
```ts
const nextConfig = {
  output: "standalone",
};
...
**Step 16.** Create a file called `.env` and add the content below to it.
```bash
